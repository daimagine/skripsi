...
(function($) {
    ...
    var methods = {
            ...
            for (var i = 0; i < rules.length; i++) {
                ...
                switch (rules[i]) {
                    ...
                    case "funcCall":
                        ...
                        break;
                    case "biggerThan":
                        errorMsg = methods._biggerThan(field, rules, i, options);
                        break;
                    case "lessThan":
                        errorMsg = methods._lessThan(field, rules, i, options);
                        break;
         ...
         * @return an error string if validation failed
         */
        _biggerThan: function(field, rules, i, options) {
            var equalsField = rules[i + 1];

            if (parseFloat(field.val()) <= parseFloat($("#" + equalsField).val()))
                return options.allrules.biggerThan.alertText + equalsField;
        },
        _lessThan: function(field, rules, i, options) {
            var equalsField = rules[i + 1];

            if (parseFloat(field.val()) >= parseFloat($("#" + equalsField).val()))
                return options.allrules.lessThan.alertText + equalsField;
        },
        /**
         * Field match
         *
